export interface Country {
  /** ISO 3166-1 alpha-2 code, also what the API stores as `phone_country`. */
  iso: string
  name: string
...
/** [iso, name, dial] — kept as tuples so the worldwide list stays readable. */
...
/** Used when the device region is unknown or not a country we have on file. */
export const DEFAULT_COUNTRY = byIso.get('KE') as Country

export function findCountry(iso?: string | null): Country | undefined {
  return iso ? byIso.get(iso.toUpperCase()) : undefined
}

/**
 * Regional indicator pair, e.g. 'KE' -> 🇰🇪. A handful of Android skins render
 * the letters instead of a flag, which still reads correctly.
 */
export function flagEmoji(iso: string): string {
  return String.fromCodePoint(...[...iso.toUpperCase()].map((char) => 0x1f1e6 + char.charCodeAt(0) - 65))
}

/** Local `0712…`/`+254712…` input to the E.164 form the API stores. */
...
export function searchCountries(query: string): Country[] {
  ...
  return countries.filter(
    (country) =>
      ...
      country.dial.startsWith(withoutPlus)
  )
}
